...
export default async function MyIdeasPage({ searchParams }: PageProps) {
  ...
  const ideasRes: ApiResponse<Idea[]> = await fetchServer(`/ideas/user/my-ideas${queryString ? `?${queryString}` : ""}`);

  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">My Ideas</h1>
          <p className="text-zinc-500 dark:text-zinc-400">View and manage all your submitted sustainability ideas.</p>
        </div>
