...
export default async function MemberDashboardPage({ searchParams }: PageProps) {
  ...
  let stats: any = {
    data: { totalIdeas: 0, approvedIdeas: 0, pendingIdeas: 0 },
  };
  let ideasRes: ApiResponse<Idea[]> = { success: false, message: "", data: [] };

  try {
    const [statsRes, listRes]: [any, ApiResponse<Idea[]>] = await Promise.all([
  ...
  const statConfig = [
    {
      label: "Total Ideas",
      value: stats?.data?.totalIdeas ?? 0,
      color: "bg-blue-500",
    },
    {
      label: "Approved",
      value: stats?.data?.approvedIdeas ?? 0,
      color: "bg-emerald-500",
    },
    {
      label: "Pending",
      value: stats?.data?.pendingIdeas ?? 0,
      color: "bg-amber-500",
    },
  ];
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-lg text-zinc-500 dark:text-zinc-400">
            Manage and track your sustainability impact.
          </p>
        </div>
      ...
      <div className="grid gap-6 sm:grid-cols-3">
        {statConfig.map((stat, i) => (
          <div
            key={i}
            className="rounded-3xl border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-950 shadow-sm relative overflow-hidden group"
          >
            <div
              className={cn(
                "absolute top-0 right-0 h-24 w-24 translate-x-12 -translate-y-12 rounded-full opacity-10 blur-2xl transition-all group-hover:opacity-20",
                stat.color,
              )}
            />
